...
export class PartitaService {
  ...
  eseguiMossa(id: number | undefined, mossa: Mossa): Observable<ScacchieraGameStateDTO> {
    ...
    return this.http.post<ScacchieraGameStateDTO>(`${this.baseUrl}/mossa/${id}`, mossa, {withCredentials : true}).pipe(
      tap((dto) => {
        // ✅ Controlli lato client sul DTO ricevuto
        if (dto.isCheckMate) {
          alert('♔ Scacco matto! La partita è finita.');
        } else if (dto.isStallo) {
          alert('😐 Stallo! La partita termina in pareggio.');
        } else if (dto.isCheck) {
          alert('⚠️ Sei sotto scacco!');
        }

        console.log('🎯 Stato partita aggiornato:', dto);
      })
    );
  }
